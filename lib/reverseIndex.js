...
// Allows the numeric suffix appended when two names slugify identically.
...
/**
 * Assign a unique slug to every entry. Entries are slugged in name order so
 * that a collision resolves to the same slug on every rebuild.
 */
 ...
 */
function buildReverseIndex(allApps) {
  const apps = {};
  ...
  for (const app of allApps || []) {
    ...
    if (trackerNames.length > 0) trackedApps++;
  ...
  assignSlugs(trackerArray);
  assignSlugs(companyArray);
  ...
  // deduplicated here.
  const trackerSlugs = {};
  for (const entry of trackerArray) trackerSlugs[entry.name.toLowerCase()] = entry.slug;
  const companySlugs = {};
  for (const entry of companyArray) companySlugs[entry.name.toLowerCase()] = entry.slug;
