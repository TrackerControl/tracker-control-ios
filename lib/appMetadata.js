...
function buildReportMetadata({
  ...
  const absentSince = firstNonEmpty(storefront && (storefront.absent_since || storefront.absentSince));
  ...
  return {
    ...
    // The listing a storefront has withdrawn no longer resolves there.
    url: absentSince ? null : firstNonEmpty(current.url, historical.url, queued.url),
    analysedVersion,
    ...
    // Set while the last check found the app missing from the storefront.
    // currentFetchedAt then dates the last listing that was found, and
    // storefrontCheckedAt the check that found it gone.
    storefrontAbsentSince: absentSince,
    storefrontCheckedAt: absentSince
      ? firstNonEmpty(storefront.checked_at, storefront.checkedAt)
      : null
  };
}
